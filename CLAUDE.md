...
This package is a Bun-managed TypeScript SDK.

- `src/index.ts`: public export barrel (entrypoint).
- `src/errors/`: custom error classes (for example, `OfficeToPdfError`).
...
- File naming:
  - Classes: `PascalCase.ts` (e.g., `OfficeToPdf.ts`).
  - Utils/types: `kebab-case.ts` (e.g., `convert-office-to-pdf.ts` if added).
